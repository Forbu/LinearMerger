...
class AirbnbDataset(Dataset):
    def __init__(self, data: pd.DataFrame):
        ...
        self.categorical_data = self.data.select_dtypes(include=["category", "int8"])
        self.numerical_data = self.data.select_dtypes(include=["float"])
...
torch.save(model.state_dict(), f"models/linearmerge_model_{uuid.uuid4()}.pth")
