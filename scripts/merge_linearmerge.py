...
import linearmerge.linearmerge_model as linearmerge_model
...
# list the two linearmerge models in models/
import os
linearmerge_models = os.listdir("models/")
linearmerge_models = [model for model in linearmerge_models if "linearmerge_model" in model]

print(linearmerge_models)

# now we want to import the two models that are in models/
models_0_state_dict = torch.load(
    f"models/{linearmerge_models[0]}"
)
models_1_state_dict = torch.load(
    f"models/{linearmerge_models[1]}"
)
...
for time_value in time_values:
    model = linearmerge_model.LinearMergeModel(
...
np.save("results/merge_linearmerge.npy", results_all)
